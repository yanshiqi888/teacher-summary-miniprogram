...
function generateStudentCommentPrompt(formData) {
  const text = "你是一位经验丰富的中学教师 需要为学生写期末评语 要求 每条评语约100字 每条评语必须以该生开头 语气鼓励温和 内容通用不涉及具体学生特征 涵盖学习态度课堂表现作业完成品德表现等方面 避免AI痕迹要像真实老师手写的评语 每条评语必须完全不同不能有任何重复或相似的表达 从不同角度描述有的侧重学习态度有的侧重课堂表现有的侧重品德有的侧重进步空间 使用不同的句式结构和表达方式 请生成" + formData.count + "条完全不同的学生评语 每条评语用换行符分隔"
  return text
}
