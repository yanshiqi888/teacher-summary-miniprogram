...
严格要求：
1. 必须生成恰好${formData.count}条评语，一条都不能少
2. 每条评语必须以"该生"两个字开头
3. 每条评语100-120字，不能太短也不能太长
4. 每条评语是完整的一段话，内部不能有换行
5. 每条评语必须完全不同，不能有相似的句式或表达
6. 从多个不同角度描述：学习态度、课堂表现、作业完成、品德修养、人际交往、特长发展、进步空间
7. 使用多样化的句式结构，避免套路化
8. 语气温和鼓励，像真实老师手写的评语
9. 内容通用，不涉及具体学生姓名或特征
...
- 评语之间用换行符分隔
- 不要添加序号、标题或其他内容
- 确保输出恰好${formData.count}条评语

请严格按照要求生成${formData.count}条评语。`
  return text
}
